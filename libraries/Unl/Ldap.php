...
class Unl_Ldap {
    ...
    public function __construct($uri)
    {
    	...
    	if(ldap_set_option($this->_conn, LDAP_OPT_PROTOCOL_VERSION, 3) === FALSE) {
    	    throw new Unl_Ldap_Exception('Could not connect to LDAP Server: ' . ldap_error($this->_conn), ldap_errno($this->_conn));
    	}

    	// check for an actual connection
    	@ldap_start_tls($this->_conn);
    	if(ldap_errno($this->_conn) == -1) {
    	    throw new Unl_Ldap_Exception('Could not connect to LDAP Server', -1);
    	}
    ...
    public function bind($rdn = null, $password = null)
    {
        if(@ldap_bind($this->_conn, $rdn, $password) === FALSE) {
            throw new Unl_Ldap_Exception('Unable to bind: ' . ldap_error($this->_conn), ldap_errno($this->_conn));
        }
    }
    ...
    public function search($base_dn, $filter=null)
    {
        ...
        if($result === FALSE) {
            throw new Unl_Ldap_Exception('LDAP search failed: ' . ldap_error($this->_conn));
        }
        ...
        if(ldap_parse_result($this->_conn, $result, $errcode, $matcheddn, $errmsg, $referrals)) {
            if($errcode !== 0) {
                throw new Unl_Ldap_Exception('Error retrieving results: ' . ldap_err2str($errcode));
            }
        } else {
            throw new Unl_Ldap_Exception('Error retrieving results: ' . ldap_error($this->_conn));
        }
